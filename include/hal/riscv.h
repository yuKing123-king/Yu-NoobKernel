...
static inline void set_sstatus_bit(u64 bits)
{
	asm volatile("csrs sstatus, %0" : : "r" (bits));
}

static inline void clr_sstatus_bit(u64 bits)
{
...
static inline void w_frm(u32 x)
{
	asm volatile("fsrm %0" : : "r" (x));
}

static inline u64 r_sstatus_fs()
{
	return r_sstatus() & SSTATUS_FS_BITS;
}

static inline void w_sstatus_fs(u64 bits)
{
	u64 status = r_sstatus() & ~SSTATUS_FS_BITS;
